...
mod_identification_server <- function(id, usr, i18n) {
    moduleServer(id, function(input, output, session) {
        ...
        observeEvent(usr$person_id, {
            ...
            output$identification_ui <- renderUI({
                tagList(
                    ...
                    selectInput(
                        ...
                        choices = c(
                            "",
                            IPCASreporter::departments$department_name
                        ),
                        multiple = FALSE
                    ),
                    ...
                    textAreaInput(
                        ...
                        placeholder = i18n()$t(
                            "Např. změny ve výši úvazku v průběhu roku nebo překážky ve výkonu prac. činnosti jako dlouhodobá nemoc, péče o dítě/osobu blízkou apod."
                        )
                    )
                )
            })

            identification$employee_name <- paste(
                persons$data$name_first,
                persons$data$name_last
            )
            ...
            output$employee_name <- renderText({
                paste(identification$employee_name)
            })
            output$department <- renderText({
                identification$department
            })
            output$fte <- renderText({
                identification$fte
            })
